/*
  pool.ts
  說明：TypeScript 版本的粒子池管理模組（非破壞性新增，保留原始 pool.js）。
*/
import { Particle } from './particle';
import { AMBIENT_COLS } from './constants';
...
export function quotaForType(mbti: string, mbtiCounts: Record<string, number>): number {
  // 計算指定 MBTI 類型的配額 （根據當前各類型的數量比例，並限制在 MIN_PER_TYPE 與 MAX_PER_TYPE 之間）
  const total = Object.values(mbtiCounts).reduce((a, b) => a + b, 0) || 1;
  const count = mbtiCounts[mbti] || 1;
  const raw = Math.floor((count / total) * MAX_MBTI_TOTAL);
  return Math.max(MIN_PER_TYPE, Math.min(MAX_PER_TYPE, raw));
...
export function pruneType(mbti: string, mbtiCounts: Record<string, number>): void {
  // 根據配額裁剪指定 MBTI 類型的顆粒，從最舊的開始移除，直到剩下的數量不超過配額
  const arr = mbtiParticles[mbti];
  if (!arr) return;
  const quota = quotaForType(mbti, mbtiCounts);
  while (arr.length > quota) {
...
export function pruneAllTypes(mbtiCounts: Record<string, number>): void {
  // 對所有 MBTI 類型執行 pruneType，確保整體顆粒數量符合配額限制
  for (const mbti of Object.keys(mbtiParticles)) pruneType(mbti, mbtiCounts);
}
...
export function seedAmbient(
  // 產生 ambient（背景）顆粒，這些顆粒不會被 MBTI 配額裁剪，數量由參數 n 決定
  n: number,
  W: number = (typeof window !== 'undefined' ? window.innerWidth : 800),
  H: number = (typeof window !== 'undefined' ? window.innerHeight : 600)
): void {
  for (let i = 0; i < n; i++) {
    const col = AMBIENT_COLS[i % AMBIENT_COLS.length]; // 循環使用預定義的 ambient 顏色
    const p: any = new (Particle as any)(Math.random() * W, Math.random() * H, col, null); // MBTI 類型為 null 表示這是 ambient 顆粒
    if (i < 3) {
      p.sizeClass = 'blob';
      p.size = 14 + Math.random() * 18;
