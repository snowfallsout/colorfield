/*
  particle.js
  說明：包含 `Particle` 類別與相關物理/視覺參數與輔助函式。
  - `Particle` 管理單一粒子的狀態、更新與繪製。
  - 匯出部分工具供其他模組使用（如 pool.js）。
*/
import { TWO_PI } from './constants.js';
import { getSpriteSet, getDotSprite } from './sprites.js';

/**
 * 各尺寸類別的參數：視覺大小、alpha 範圍、阻力等
 * @type {{dot:Object,blob:Object,field:Object}}
 */
export const SIZE_CLASS_PARAMS = {
...
/**
 * 隨機選擇粒子尺寸類別（field/blob/dot），帶有機率分佈。
 * @returns {'field'|'blob'|'dot'}
 */
export function pickSizeClass() {
  const r = Math.random();
...
export class Particle {
  ...
  /**
   * 更新粒子物理與行為狀態
   * @param {Array} faces - 臉部資訊陣列
   * @param {string} emotion - 全域情緒狀態
   * @param {Array} activePinchPoints - 當前捏合點陣列
   * @param {Object} mbtiParticles - 依 MBTI 分類的粒子索引
   * @param {number} W - 畫布寬度
   * @param {number} H - 畫布高度
   */
  update(faces, emotion, activePinchPoints, mbtiParticles, W, H) {
 ...
 * @param {number} alpha
 */
export function drawDiamondSparkle(ctx, x, y, size, alpha) {
 ...
 * 計算二維平方距離（避免頻繁開根號）
 */
export function dist2sq(ax, ay, bx, by) { const dx = ax-bx, dy = ay-by; return dx*dx + dy*dy; }

/**
 * 從 faces 陣列中找到距離指定座標最近的臉部
 * @param {Array} faces
 * @param {number} px
 * @param {number} py
 * @returns {Object|null}
 */
export function nearestFace(faces, px, py) {
